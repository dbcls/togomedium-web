...
class GraphManager {
    ...
    redraw(data) {
        ...
        const newLinks = links
            ...
            .attr("stroke", (d) => w(THEME.COLOR.ACCENT)
...
const getInitialData = async (gm_id) => {
    const links = [];
...
const getSimilarData = async (gm_id) => {
    const apiName = "gmdb_list_similar_media_by_gmid";
    const data = await getData(`${URL_API}${apiName}`, {
        gm_id,
        limit: 99,
        offset: 0,
    });
...
const StanzaView = ({ gmId }) => {
    const { graphData } = useGraphData(gmId);
    reactExports.useEffect(() => {
        console.log(graphData);
    }, [graphData]);
    return (jsx(StanzaWrapper, { children: jsx(NodeCanvas, { data: graphData }) }));
};
...
var metadata = {
	...
	"stanza:label": "[WIP] Similar media node",
	"stanza:definition": "",
	"stanza:type": "Stanza",
	"stanza:display": "Text",
	"stanza:provider": "",
